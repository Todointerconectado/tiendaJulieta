...
const inputs = document.querySelectorAll('#from input');
const expresiones_regulares = {
    user:     /^[\w\-]{4,14}$/        , // Letras, números, guion y guion_bajo.
    name:     /^[a-zA-ZÀ-ÿ\s]{3,30}$/ , // Letras, espacios, pueden llevar acentos.
    password: /^[\w\W]{4,14}$/        , // 4 a 12 digitos.
...
let validar_from = (event) => {
    switch (event.target.name) {
        case "user":
            validar_campo(expresiones_regulares.user, event.target,'user');
            break;
        case "name":
            validar_campo(expresiones_regulares.name, event.target,'name');
            break;
        case "password":
            validar_campo(expresiones_regulares.password, event.target,'password');
            validar_password2();
            break;
        case "password2":
            validar_password2();
...
from.addEventListener('submit', event => {
    ...
    if(campos.user && campos.name && campos.password && campos.email && campos.telefono && terminos.checked ) {
        from.reset();
